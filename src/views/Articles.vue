                ...
                <div class="relative z-10">
                  ...
                  <div class="mb-2">
                    <h3 class="text-xl font-bold text-white dark:text-white font-serif leading-tight group-hover:text-amber-300 dark:group-hover:text-amber-300 transition-colors duration-300 line-clamp-2">
                      <router-link 
                        v-if="article.articleManuscriptId || article.articleId"
                        :to="`/articles/${article.articleManuscriptId || article.articleId}`" 
                        class="hover:underline block"
                      >
                        {{ article.articleTitle }}
                      </router-link>
                      <span v-else>{{ article.articleTitle }}</span>
                    </h3>
                  </div>
                  ...
                  <div class="flex justify-between items-center mt-3">
                    ...
                    <div class="flex items-center gap-4">
                      ...
                      <span v-else class="inline-flex items-center text-slate-400 dark:text-slate-400 text-sm">
                        Read Full Article
                      </span>
  ...
  try {
    ...
    if (selectedYear.value) {
      list = list.filter(article => {
        const publishTime = article.articleOnlineTime || article.articlePublishTime
        if (!publishTime) return false
        const articleYear = new Date(publishTime).getFullYear().toString()
        return articleYear === selectedYear.value
      })
    }
    ...
    if (selectedUserId.value) {
      list = list.filter(article => {
        if (!article.articleAuthors || !Array.isArray(article.articleAuthors)) return false
        return article.articleAuthors.some(author => {
          return author.authorUser?.userId === selectedUserId.value
        })
      })
    }
...
const initializeFromRoute = () => {
  ...
  if (userIdParam && typeof userIdParam === 'string') {
    selectedUserId.value = userIdParam
  }
}
...
// 监听路由变化，当从ArticleDetails跳转过来时更新作者筛选
